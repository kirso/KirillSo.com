...
export const markup = (title: string, pubDate: string) => ({
	type: "div",
	props: {
...
export async function generateOgImage(title: string, pubDate: string) {
	const svg = await satori(markup(title, pubDate), ogOptions);
